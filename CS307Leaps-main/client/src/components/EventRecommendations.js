...
const EventRecommendations = ({ results, onAddToTrip, currentTrip }) => {
  ...
  const containsItem = (item) => {
    const ret = currentTrip.items.some(tripItem => tripItem.item_id === item.id);
    console.log("item id: %s", item.id);
    //console.log("first tripitem id: %s", currentTrip.items[0].item_id)
    console.log("item present: %s", ret);
    return ret;
  }
          ...
          items.length > 0 && (
            <div key={type}>
              ...
              {items.map(item => (
                ...
                >
                  {!containsItem(item) && (<div>
                    {renderItem(item, type)}
                    {!containsItem(item) && (<button
                      onClick={(e) => {
                        e.stopPropagation();
